...
public:
  CreateMeshCommand(Scene *scene, const std::string &name,
  ...
  void Execute() override {
    ...
    // Check if it already exists (sanity check, though Execute implies it
    // shouldn't)
    ...
    SceneAPI::CreateMeshEntityWithUUID(*m_Scene, m_EntityUUID, m_Name, m_Mesh,
                                       m_Position);
  }
  ...
  std::string GetDescription() const override { return "Create " + m_Name; }
...
public:
  DeleteEntityCommand(Scene *scene, Entity entity) : m_Scene(scene) {
  ...
  void Undo() override {
    ...
    Entity restored =
        m_Scene->CreateEntityWithUUID(m_EntityUUID, m_TagComp.Tag);

    // Restore TagComponent (CreateEntityWithUUID does not add it by default to
    // avoid conflicts)
...
public:
  ModifyTransformCommand(Scene *scene, Entity entity,
                         const TransformComponent &oldFn,
                         const TransformComponent &newFn)
      : m_Scene(scene), m_OldTransform(oldFn), m_NewTransform(newFn) {
...
public:
  RenameEntityCommand(Scene *scene, Entity entity, const std::string &oldName,
...
public:
  DuplicateEntityCommand(Scene *scene, Entity source, bool isLinked = false)
      : m_Scene(scene), m_SourceUUID(source.GetComponent<IDComponent>().ID),
        m_IsLinked(isLinked) {
  ...
  void Execute() override {
    ...
    SceneAPI::DuplicateEntityWithUUID(*m_Scene, source, m_NewEntityUUID,
                                      m_IsLinked);
  }
...
public:
  ReorderEntityCommand(Scene *scene, Entity entity, int32_t oldOrder,
                       int32_t newOrder)
  ...
  std::string GetDescription() const override { return "Reorder Entity"; }
...
// =========================================================================================
// CREATE CAMERA COMMAND (LOW-06 FIX: Camera creation is now undoable)
// =========================================================================================
class CreateCameraCommand : public ICommand {
public:
  CreateCameraCommand(Scene *scene, const std::string &name = "Camera")
  ...
  void Execute() override {
    ...
    // Generate a unique name at creation time (handles "Camera", "Camera (1)", etc.)
    std::string uniqueName = SceneAPI::GenerateUniqueName(*m_Scene, m_Name);

    Entity camera = m_Scene->CreateEntityWithUUID(m_EntityUUID, uniqueName);
    camera.AddComponent<TagComponent>(uniqueName);
    camera.AddComponent<TransformComponent>();
    camera.AddComponent<CameraComponent>();
    SceneAPI::SetNextOrder(camera);
  }
