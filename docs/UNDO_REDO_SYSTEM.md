...
- **State Capture**: Snapshots the _exact_ `TransformComponent` (Position, Rotation, Scale) before and after the operation.
...
- **Optimization**: Implemented **Key Debouncing** (`m_UndoPressedLastFrame`). The system only accepts the _rising edge_ of the key press, guaranteeing exactly **one Undo step per key press**.
